...
plot <- ggplot(df, aes(x=n)) +
        ...
        scale_linetype_manual("Boot peer Selection" ,labels = c("Randomly picked", "Successor"), values = c("F1", "twodash")) +
        #ggtitle("Strategies of recommending Boot peer ") +
        labs(y = "Join network time (ms)", x = "Number of peers in network") +
        ggsave(file="Strategies of recommending Boot peer.png",  width = 15.875, height=11.985625, units = "cm", dpi=300)
