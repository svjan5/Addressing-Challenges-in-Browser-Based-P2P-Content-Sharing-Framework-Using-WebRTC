...
plot <- ggplot(df, aes(x=n)) +
        ...
        scale_linetype_manual("Scenarios" ,labels = c("With fingers: O(log n)", "Without fingers: O(n)"), values = c("dashed", "solid")) +
        ggtitle("Performance of join network operation") +
        labs(y = "Number of messages exchanged", x = "Number of peers in network")+
        ggsave(file="A_plot.png",  width = 15.875, height=11.985625, units = "cm", dpi=300)
